...
public class NetBankingAdapter  implements PaymentAdapter {
    ...
    public PaymentGatewayResponse initiate(PaymentGatewayRequest request) {
        ...
        try {
            ...
            log.error("NetBanking Failed, PaymentId : {} ", request.paymentId());

            return new PaymentGatewayResponse.Failure("NBK_FAILED", e.getMessage());
        }
    }

    @Override
    public PaymentGatewayResponse capture(UUID paymentId) {
        return new PaymentGatewayResponse.Success("NBK_REF");
    }
}
